...
async def get_active_orders_lawyer(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if orders:
        ...
        for i, order in enumerate(orders):
            kb.button(text=f'{i + 1}', callback_data=f'order_send_info_{order}')
...
async def on_lawyer_client_chat_start(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    order_id = call.data.split('_')[-1]
    if order_id is None:
        await call.answer(text='Исполнитель еще не выбран.', show_alert=True)
        return
...
async def on_full_end(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    lawyer_kb = await get_main_lawyer_kb()
    await call.message.answer(text=text_to_lawyer, reply_markup=lawyer_kb)


async def ask_card_num(call: CallbackQuery, bot: Bot, state: FSMContext):
    await call.answer()
    order_id = call.data.split("_")[-1]
    await state.update_data(ORDER_ID=order_id)
    text_get_card = "Высылайте номер карты, на который необходимо провести расчет:"
    await state.set_state(Consult.CARD_NUM)
    await call.message.answer(text=text_get_card)


async def get_card_num(message: Message, bot: Bot, state: FSMContext):
    num = message.text
    await state.update_data(CARD_NUM=num)
    text = f"Вы ввели: {num}, отправить на обработку?\nЛибо отправьте верный номер."
    kb = InlineKeyboardBuilder()
    kb.button(text="Отправить", callback_data="send_card_info")
    await message.answer(text=text, reply_markup=kb.as_markup())


async def process_card_num(call: CallbackQuery, bot: Bot, state: FSMContext):
    await call.answer()
    data = await state.get_data()
    num = data.get("CARD_NUM")
    order_id = data.get("ORDER_ID")
    ...
    lawyer_kb = await get_main_lawyer_kb()
    await bot.send_message(chat_id=call.from_user.id, text="Данные отправлены.", reply_markup=lawyer_kb)
