...
from handlers.admin import get_chat
from handlers.lawyers import router_lawyers, process_response, get_develop_time, get_develop_price, \
    ...
    on_get_end_info, on_full_end, ask_card_num, get_card_num, process_card_num
...
from handlers.users import router_users, get_start, process_input_file, clear_state, process_file_from_user, \
    ...
    on_get_dispute_text, on_get_query_support_text, on_send_query_to_support, process_query_support
...
async def register_users():
    ...
    router_users.callback_query.register(on_lawyer_client_chat_start,  F.data.startswith("question_lawyer_"))
    ...
    """ВОПРОС В ПОДДЕРЖКУ"""
    router_lawyers.callback_query.register(process_query_support, F.data.startswith('query_to_support'))
    router_lawyers.message.register(on_get_query_support_text, StateFilter(Consult.QUERY_SUPPORT))
    router_lawyers.callback_query.register(on_send_query_to_support, F.data.startswith('send_query_support'))
...
async def register_lawyers():
    ...
    router_lawyers.callback_query.register(send_order_info, F.data.startswith('order_send_info'))
    router_lawyers.callback_query.register(ask_card_num, F.data.startswith('get_card_'))
    router_lawyers.callback_query.register(process_card_num, F.data.startswith('send_card_info'))
    router_lawyers.message.register(get_card_num, F.text, StateFilter(Consult.CARD_NUM))
    ...
    router_lawyers.message.register(get_chat, Command("chat"))
